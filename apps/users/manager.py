...
class CustomUserManager(BaseUserManager):
    ...
    def create_user(
        ...
        raise_statement = (
            "Users must submit a username"
            if not username
            else (
                "Users must submit a first_name"
                if not first_name
                else "Users must submit a last_name" if not last_name else None
            )
        )
        if raise_statement:
            raise ValueError(_(raise_statement))
